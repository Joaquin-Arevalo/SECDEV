...
document.addEventListener("DOMContentLoaded", function(){
    fetch("/man_retrieve_employee_total_wp")
    .then(response =>{
        if (!response.ok){
...
function dropdown(){
    ...
    selectedWeek.addEventListener('change', function(){
        ...
        fetch(`/man_retrieve_emp_wpay?employee=${selected_emp}&week=${selectedWeek2}`)
        .then(response =>{
            if (!response.ok){
                throw new Error(`HTTP error! Status: ${response.status}`);
            }
        ...
        .catch(error =>{
            console.error('Error fetching /man_retrieve_emp_wpay:', error);
        });
    })   
    ...
    function fetch_weekly_payroll(){
        fetch(`/man_retrieve_emp_wpay?employee=${curr_emp}&week=${week_index}`)
        .then(response =>{
            if (!response.ok){
                throw new Error(`HTTP error! Status: ${response.status}`);
            }
